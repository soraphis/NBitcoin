...
{
	class ECMultiplicationContext
	{

		static readonly Lazy<ECMultiplicationContext> _Instance = new Lazy<ECMultiplicationContext>(CreateInstance, true);
		static ECMultiplicationContext CreateInstance()
		{
			return new ECMultiplicationContext();
		}
		public static ECMultiplicationContext Instance => _Instance.Value;
		...
		public ECMultiplicationContext()
		{
			...
			/* get the generator */
			GroupElementJacobian gj = EC.G.ToGroupElementJacobian();
			this.pre_g = new GroupElementStorage[ArraySize];

			/* precompute the tables with odd multiples */
			secp256k1_ecmult_odd_multiples_table_storage_var(ArraySize, pre_g, gj);

			{
				GroupElementJacobian g_128j;
				int i;

				this.pre_g_128 = new GroupElementStorage[ArraySize];
		...
		static void secp256k1_ecmult_odd_multiples_table_storage_var(int n, GroupElementStorage[] pre, in GroupElementJacobian a)
		{
			GroupElementJacobian d;
			GroupElement d_ge, p_ge;
			GroupElementJacobian pj;
			FieldElement zi;
			FieldElement zr;
			FieldElement dx_over_dz_squared;
			int i;
			 ...
			 */
			d_ge = new GroupElement(d.x, d.y, false);


			p_ge = a.ToGroupElementZInv(d.z);
			pj = new GroupElementJacobian(p_ge.x, p_ge.y, a.z, false);

			for (i = 0; i < (n - 1); i++)
			{
				pj = new GroupElementJacobian(pj.x, pj.y.NormalizeVariable(), pj.z);
				pre[i] = new GroupElementStorage(pre[i].x, pj.y.ToStorage());
				pj = pj.AddVariable(d_ge, out zr);
				zr = zr.NormalizeVariable();
				pre[i] = new GroupElementStorage(zr.ToStorage(), pre[i].y);
			}
			...
			d = new GroupElementJacobian(d.x, d.y, zi * pj.z, d.infinity); /* d.z = 1/d.z */
			...
			while (i > 0)
			{
				 ...
				 * point from that. */
				ref readonly FieldElement rzr = ref p_ge.x;
				zi = zi * rzr;
				zi2 = zi.Sqr();
				zi3 = zi2 * zi;
				...
				p_ge = new GroupElement(p_gex, p_gey, p_ge.infinity);
				/* Store */
				pre[i] = p_ge.ToStorage();
			}
		}

		public GroupElementJacobian ECMultiply(in GroupElementJacobian a, in Scalar na, in Scalar ng)
		{
			Span<Scalar> nas = stackalloc Scalar[1];
			nas[0] = na;
			Span<GroupElementJacobian> @as = stackalloc GroupElementJacobian[1];
			@as[0] = a;
			unsafe
			{
				GroupElementJacobian* prej = stackalloc GroupElementJacobian[ArraySize_A];
				FieldElement* zr = stackalloc FieldElement[ArraySize_A];
				GroupElement* pre_a = stackalloc GroupElement[ArraySize_A];
				GroupElement* pre_a_lam = stackalloc GroupElement[ArraySize_A];
		...
		unsafe GroupElementJacobian ECMultiply(in StraussState state, in Span<GroupElementJacobian> a, in Span<Scalar> na, in Scalar? ng, int num)
		{
			var r = GroupElementJacobian.Infinity;

			// secp256k1_ecmult_strauss_wnaf
			GroupElement tmpa;
			FieldElement Z = FieldElement.Zero;

			var pre_a_span = new Span<GroupElement>(state.pre_a, num * ArraySize_A);
			var pre_a_lam_span = new Span<GroupElement>(state.pre_a_lam, num * ArraySize_A);
			var prej_span = new Span<GroupElementJacobian>(state.prej, num * ArraySize_A);
			var zr_span = new Span<FieldElement>(state.zr, num * ArraySize_A);
     ...
     */
			if (no > 0)
			{
				...
				for (np = 1; np < no; ++np)
				{
					GroupElementJacobian tmp = a[ps_span[np].input_pos];

					ref GroupElementJacobian prej_span_1 = ref prej_span[(np - 1) * ArraySize_A + ArraySize_A - 1];
#if SECP256K1_VERIFY
					prej_span_1 = new GroupElementJacobian(prej_span_1.x, prej_span_1.y, prej_span_1.z.NormalizeVariable(), prej_span_1.infinity);
#endif
			...
			if (!r.infinity)
			{
				r = new GroupElementJacobian(r.x, r.y, r.z * Z);
			}

			return r;
		}

		static void secp256k1_ge_globalz_set_table_gej(int len, Span<GroupElement> r, ref FieldElement globalz, Span<GroupElementJacobian> a, Span<FieldElement> zr)
		{
			int i = len - 1;
			FieldElement zs;
			...
			if (len > 0)
			{
				...
				r[i] = new GroupElement(a[i].x, a[i].y.NormalizeWeak(), false);
				globalz = a[i].z;
				zs = zr[i];
 ...
 */
		internal static void secp256k1_ecmult_odd_multiples_table(int n, Span<GroupElementJacobian> prej, Span<FieldElement> zr, in GroupElementJacobian a)
		{
			...
			prej[0] = new GroupElementJacobian(a_ge.x, a_ge.y, a.z, false);
			 ...
			 */
			prej[n - 1] = new GroupElementJacobian(prej[n - 1].x, prej[n - 1].y, prej[n - 1].z * d.z, prej[n - 1].infinity);
		}

		[MethodImpl(MethodImplOptions.NoOptimization | MethodImplOptions.AggressiveInlining)]
		private static void ECMULT_TABLE_GET_GE_STORAGE(out GroupElement r, in Span<GroupElementStorage> pre, int n, int w)
		{
			VERIFY_CHECK(((n) & 1) == 1);
			...
			{
				r = (pre)[(-(n) - 1) / 2].ToGroupElement();
				r = new GroupElement(r.x, r.y.Negate(1), r.infinity);
			}
		}

		[MethodImpl(MethodImplOptions.NoOptimization | MethodImplOptions.AggressiveInlining)]
		private static void ECMULT_TABLE_GET_GE(out GroupElement r, in Span<GroupElement> pre, int n, int w)
		{
			VERIFY_CHECK(((n) & 1) == 1);
			VERIFY_CHECK((n) >= -((1 << ((w) - 1)) - 1));
			VERIFY_CHECK((n) <= ((1 << ((w) - 1)) - 1));
